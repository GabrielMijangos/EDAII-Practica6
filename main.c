...
//gcc -Wall -std=c99 -osalida62m.out main.c List.c
//./salida62m.out
...
void Graph_Print( Graph* g, int depth )
{
   if(g->type == eGraphType_UNDIRECTED)
   {
      for( int i = 0; i < g->len; ++i )
      {
         ...
         printf("\n[%d]El aeropuerto con id %d tiene los siguientes datos: \n", i, vertex->data.id);
         printf("---Tiempo UTC: %d \n", vertex->data.utc_time);
         printf("---Códifo IATA: %s \n", vertex->data.iata_code);
         printf("---País: %s \n", vertex->data.country);
         printf("---Ciudad: %s \n", vertex->data.city);
         printf("---Nombre: %s \n", vertex->data.name);
         ...
         if( vertex->neighbors )
         {
               ...
               List_Cursor_next( vertex->neighbors ) )
            {
               ...
               printf( "------El aeropuerto con IATA %s\n", g->vertices[ neighbor_idx ].data.iata_code);
            }
         }
         printf( "y nada más.\n" );
      }
      ...
      for( int i = 0; i < g->len; ++i )
      {
         ...
         if( vertex->neighbors )
         {
               ...
               List_Cursor_next( vertex->neighbors ) )
            {
               ...
               printf( "------El aeropuerto con IATA %s con un tiempo de %0.2f\n", g->vertices[ neighbor_idx ].data.iata_code,d.weight);
            }
         }
         printf( "\n" );
      }
...
int main()
{
  Graph *grafo = Graph_New(MAX_VERTICES, // cantidad máxima de vértices
   ...
   Graph_AddWeightedEdge(grafo, 140, 120, 3.0);
   Graph_AddWeightedEdge(grafo, 140, 160, 1.6);
   Graph_AddWeightedEdge(grafo, 150, 120, 2.5);
   Graph_AddWeightedEdge(grafo, 150, 160, 3.0);
   Graph_AddWeightedEdge(grafo, 150, 130, 1.5);
   Graph_AddWeightedEdge(grafo, 160, 140, 3.5);
   ...
   printf( "\nLos aviones en el aeropuerto con ID %d pueden ir a: \n",vertexbuscado);
   for(List_Cursor_front(vertex->neighbors);
      !List_Cursor_end(vertex->neighbors);
      List_Cursor_next(vertex->neighbors))
   {
      ...
      printf( "---El aeropuerto con IATA %s con un tiempo de %0.2f\n", grafo->vertices[ neighbor_idx ].data.iata_code,d.weight);
   }
